...
for (let i = 0; i < props.config.Rows; i += 2) {
  svg.append("line")
    ...
    if (props.gameMode) {
      ...
      const centerX = props.posX * cellSize.value + marginLeft.value + cellSize.value / 2;
      const centerY = props.posY * cellSize.value + marginTop.value + cellSize.value / 2;
...
function drawPiePiece(svg, cx, cy, radius, startAngle, endAngle, color) {
  ...
  const selection = d3.select(board.value);
  selection.selectAll(".pawn").remove(); // czyści poprzednie elementy

  // 🔁 Grupowanie pionków po pozycji
  const groupedPawns = {};
  props.pawnPositions?.forEach(pawn => {
    const key = `${pawn.x},${pawn.y}`;
    if (!groupedPawns[key]) groupedPawns[key] = [];
    groupedPawns[key].push(pawn);
  });

  // 🧩 Rysowanie sektorów
  Object.entries(groupedPawns).forEach(([key, pawns]) => {
    const [gridX, gridY] = key.split(',').map(Number);

    const cx = marginLeft.value + gridX * cellSize.value + cellSize.value / 2;
    const cy = marginTop.value + gridY * cellSize.value + cellSize.value / 2;
    const radius = cellSize.value / 2.5;
    const angleStep = (2 * Math.PI) / pawns.length;

    pawns.forEach((pawn, i) => {
      const startAngle = i * angleStep;
      const endAngle = (i + 1) * angleStep;
      drawPiePiece(selection, cx, cy, radius, startAngle, endAngle, pawn.color || "gray");
    });
  });
};
    ...
    watch([() => props.posX, () => props.posY], () => {
      ...
      watch([() => props.pawnCount, () => props.pawnPositions], () => {
        drawPawns();
    }, { deep: true });
